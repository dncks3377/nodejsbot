...
const token = 'NzUxMDI4ODA5NDYxMTM3NDM4.X1DIFQ._aqxc9fjhhaom-awVt8-qs0sGQY';
  ...
  if(message.channel.id == '753189814639263804') {
    if (message.content.includes("https://")) {
      ...
      message.channel.send({embed: {
        color: 3447003,
        title: "Warning !",
        description: `**<@${message.author.id}>**님의 메세지를 삭제하였습니다.`
      }})
    }
    ...
    if(message.content.startsWith('!1235484831548')) {
      ...
      if(message.channel.type != 'dm' && checkPermission(message)) return
       ...
       if(message.content.startsWith('!48665212')) {
        if(message.channel.type == 'dm') {
          return message.reply({embed: {
            color: 15158332,
            description: "**DM에서 사용하실 수 없는 명령어입니다 !**"
        ...
        if(message.member != null) { // 채널에서 공지 쓸 때
          let contents = message.content.slice('!48665212'.length);
          let img = 'https://storage1.upload.pe/storage/2020/10/02/ejQjbMPQrB.jpg';
          let embed = new Discord.RichEmbed()
            .setAuthor('Free', img)
            .setDescription('봇에 피드백 기능이 생겼습니다.\n\n`!피드백`으로 원하시는 자료 또는 기능들을 말씀해주세요.\n\n아래에 보이시는 것과 같이 누가 썻는지 다 보입니다.\n\n욕설 또는 장난식으로 하지말아주세요.')
            .setColor('#7f4dff')
            .setImage(URL = 'https://storage1.upload.pe/storage/2020/10/03/ZDvVkAME30.png')
            .setFooter(`[ 관리자 ]`)
            .setTimestamp()
