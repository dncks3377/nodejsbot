...
client.on('message', (message) => {
  ...
  if(message.content == '/수배') {
    embed=discord.Embed(title="오현수를 수배합니다", color=0xff0000)
    embed.set_author(name="```신태일봇 관리자```")
    embed.set_footer(text="찐따방일동")
    await self.bot.say(embed=embed)
 }
 

  if(message.content == '&태일아 마크') {
    let img = '';
  ...
  if(message.content == '&태일아 정보') {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .addField('```태일아 마크주소```', '마인크래프트 주소를 알려줍니다')
      .addField('```태일아 마크```', '태일이가 마인크래프트 다운로드 주소를 줍니다')
      .addBlankField()
      .setTimestamp()
      .setFooter('신태일', img)
