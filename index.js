...
client.on('message', (message) => {
  ...
  if(message.content == 'd 짱구') {
    ...
    let embed = new Discord.RichEmbed()
      .addField('**짱구의 정보**', '이마나 뒤통수가 남달리 크게 튀어나온 머리통. 또는 그런 머리통을 가진 사람\n북한에서는 남북머리라 한다. 이것에서 확장되어 "짱구를 굴린다" 등 머리 자체를 가리키는 말으로도 쓰인다.\n[신짱구/노하라 신노스케](https://namu.wiki/w/%EB%85%B8%ED%95%98%EB%9D%BC%20%EC%8B%A0%EB%85%B8%EC%8A%A4%EC%BC%80): 별도의 수식어 필요없이 ` 짱구 `라 하면 대다수가 이 캐릭터를 가리킨다.')
      .setColor('#ff0000')
  ...
  if(message.content == 'd 정보') {
    ...
    let embed = new Discord.RichEmbed()
      ...
      .setThumbnail(img)
      .addBlankField()
      .addField('**```안녕```**', '봇이 대답해줍니다.')
      .addBlankField()
      .addField('**```오현수```**', '오현수의 정보를 보여줍니다.')
      .addBlankField()
      .addField('**```백재민```**', '백재민의 정보를 보여줍니다.')
      .addBlankField()
      .addField('**```울지마```**', '태일이가 선물을 줍니다')
      .addBlankField()
      .addField('**```안녕```**', '태일이가 대답해줍니다')
      .addBlankField()
      .addField('**```쟤가 나 놀려```**', '태일이가 맞써 싸워줍니다')
      .addBlankField()
      .addField('**```오버워치패치법```**', '척척박사태일이가 오버워치 디텍 패치법을 알려줍니다')
      .addBlankField()
      .addField('**```동영상```**', '오현수의 마크일대기 영상주소를 올려줍니다')
      .addBlankField()
      .addField('**```마크주소```**', '마인크래프트 주소를 알려줍니다')
      .addBlankField()
      .addField('**```마크```**', '태일이가 마인크래프트 다운로드 주소를 줍니다')
      .addBlankField()
      .addField('**```토크온```**', '토크온 하는 오현수의 영상을 보여줍니다')
      .addBlankField()
      .setTimestamp()
      .setFooter('', img)
